...
def create_vit(model_cfg):
    ...
    if attn_type == 'softmax':
        ...
        from segm.model.vit_rpc import VisionTransformer
        if 'alpha' in model_cfg.keys():
            model_cfg.pop('alpha')
        model = VisionTransformer(**model_cfg)
    ...
    if backbone == "vit_base_patch8_384":
        ...
        deit_backbone = "/root/checkpoints/rpc_tiny/sym_baseline.pth"
        rpc_backbone = "/root/checkpoints/rpc_tiny/6itperlayer1.pth"
        ...
        if attn_type == 'softmax':
            ...
            checkpoint = torch.load(rpc_backbone, map_location="cpu")
...
def create_decoder(encoder, decoder_cfg):
    ...
    if "linear" in name:
        ...
        if attn_type == 'softmax':
            ...
            from segm.model.decoder_rpc import MaskTransformer
            if 'alpha' in decoder_cfg.keys():
                decoder_cfg.pop('alpha')
            decoder = MaskTransformer(**decoder_cfg)
    else:
